---
name: bb-brand-identity
description: "Activate for logo decisions, color palette, typography, visual language, and identity system questions. Also activates when someone asks 'what should we look like' or when visual direction needs to be evaluated or challenged."
---
...
## Critical Reflex: Before Any Visual Decision
...
## Identity System Framework

### Logo
- **Mark vs wordmark vs combination** — decide based on brand stage and use cases, not preference
- Minimum viable logo: works at 16px favicon, works in black/white, works reversed on dark
- Test in context before approving in isolation
- Avoid: literal icons, overly complex marks, trend-dependent aesthetics

### Color
**Primary palette:** 1-2 colors maximum. These carry brand recognition.
**Secondary palette:** Supporting colors for UI, data, content hierarchy.
**Functional colors:** Error, success, warning — separate from brand palette.

Evaluate every color against:
- Accessibility (WCAG contrast ratios for digital use)
- Competitor landscape (avoid owning what someone else already owns)
- Cultural associations in key markets
- Reproduction across media (screen, print, embroidery, signage)

### Typography
Define minimum 2 roles:
- **Display / Headline:** Personality-forward, used at large sizes
- **Body / UI:** Readability-forward, used at small sizes

Selection criteria:
- Character at large scale
- Legibility at small scale
- Licensing for all intended uses
- Language support if multi-market

### Visual Language
Beyond logo and color — the system that makes every touchpoint recognizable:
- Layout principles (grid logic, spacing philosophy)
- Imagery style (photography direction, illustration style, or no imagery)
- Iconography style
- Motion principles (if needed)
- Pattern / texture (if in scope)
...
Identity feeds into:
- `bb-design-system` — tokenizing the identity for scalable use
- `bb-visual-production` — applying identity to key visuals
- `bb-digital-product` — identity in UI context
- `bb-presentation` — identity presentation to client
